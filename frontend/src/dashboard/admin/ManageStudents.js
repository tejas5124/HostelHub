...
const ManageStudents = () => {
  ...
  useEffect(() => {
    const fetchBookingRequests = async () => {
      ...
      try {
        const hostelsResponse = await api.get(`/owner-hostels/${ownerId}`);
        const hostelsData = hostelsResponse.data;
        const ownerHostelIds = hostelsData.map(h => h.hostel_id);

        const bookingsResponse = await api.get(`/owner-bookings/${ownerId}`);
        const allBookings = bookingsResponse.data;

        const filteredBookings = allBookings.filter(b => ownerHostelIds.includes(b.hostel_id));

        setBookingRequests(filteredBookings);
        setFilteredRequests(filteredBookings);

        const hostels = [...new Set(filteredBookings.map(b => b.hostel_name))];
  ...
  const handleBookingStatus = async (bookingId, status) => {
    try {
      const response = await api.put(`/update-booking-status/${bookingId}`, { status });
      if (response.status === 200) {
        Swal.fire({
          ...
          text:
            status === 'approved'
              ? 'Booking approved. Student will be notified to visit the hostel for payment.'
              : 'Booking request rejected.',
        });

        const ownerId = localStorage.getItem('owner_id');
        const updatedBookings = await api.get(`/owner-bookings/${ownerId}`);
        setBookingRequests(updatedBookings.data);
        setFilteredRequests(updatedBookings.data);
      }
    } catch (error) {
      Swal.fire({
        icon: 'error',
  ...
  const handleFilterChange = (type, value) => {
    ...
    if (updatedFilters.hostel !== 'all') {
      filtered = filtered.filter(b => b.hostel_name === updatedFilters.hostel);
    }

    if (updatedFilters.status !== 'all') {
      filtered = filtered.filter(b => b.booking_status === updatedFilters.status);
    }
    ...
    if (updatedFilters.dateRange !== 'all') {
      ...
      filtered = filtered.filter(b => {
        ...
        if (updatedFilters.dateRange === 'last30') return checkIn >= thirtyDaysAgo;
        if (updatedFilters.dateRange === 'upcoming') return checkIn > today;
        if (updatedFilters.dateRange === 'past') return checkIn < today;
        return true;
      });
    }
        ...
        <div className="booking-requests">
          ...
          <div className="filters-section">
            <div className="filter-group">
              ...
              <select id="hostel-filter" value={filters.hostel} onChange={(e) => handleFilterChange('hostel', e.target.value)}>
                <option value="all">All Hostels</option>
                {uniqueHostels.map((hostel, i) => (
            ...
            <div className="filter-group">
              ...
              <select id="date-filter" value={filters.dateRange} onChange={(e) => handleFilterChange('dateRange', e.target.value)}>
          ...
          {filteredRequests.length > 0 ? (
            <div className="requests-grid">
              {filteredRequests.map((b) => (
                <div key={b.booking_id} className="request-card">
                  <div className="request-header">
                    <h3>{b.hostel_name}</h3>
                    <span className={`status-badge ${b.booking_status}`}>{b.booking_status}</span>
                  </div>
                  <div className="request-details">
                    <div className="detail-item"><span className="detail-label">Student Name:</span><span className="detail-value">{b.student_name}</span></div>
                    <div className="detail-item"><span className="detail-label">Email:</span><span className="detail-value">{b.student_email}</span></div>
                    <div className="detail-item"><span className="detail-label">Phone:</span><span className="detail-value">{b.student_phone}</span></div>
                    <div className="detail-item"><span className="detail-label">Check-in:</span><span className="detail-value">{new Date(b.check_in_date).toLocaleDateString()}</span></div>
                    <div className="detail-item"><span className="detail-label">Check-out:</span><span className="detail-value">{b.check_out_date ? new Date(b.check_out_date).toLocaleDateString() : 'Long-term stay'}</span></div>
                    <div className="detail-item"><span className="detail-label">Monthly Rate:</span><span className="detail-value">₹{b.total_amount}</span></div>
                    <div className="detail-item"><span className="detail-label">Available Rooms:</span><span className="detail-value">{b.available_rooms}</span></div>
                  </div>
                  {b.booking_status === 'pending' && (
                    <div className="request-actions">
                      <button className="approve-btn" onClick={() => handleBookingStatus(b.booking_id, 'approved')} disabled={b.available_rooms <= 0}>
                        Approve
                      </button>
                      <button className="reject-btn" onClick={() => handleBookingStatus(b.booking_id, 'rejected')}>
                        Reject
                      </button>
                    </div>
                  )}
                  {b.booking_status === 'approved' && (
                    <div className="booking-message"><p>Student will visit hostel for payment</p></div>
                  )}
                </div>
              ))}
            </div>
