...
const ApproveHostel = () => {
  ...
  useEffect(() => {
    const fetchHostels = async () => {
      try {
        const response = await api.get('/all-hostels');
  ...
  const handleApprove = async (hostelId) => {
    try {
      await api.post(`/api/hostels/approve/${hostelId}`);
  ...
  const handleReject = async (hostelId) => {
    try {
      await api.post(`/api/hostels/reject/${hostelId}`);
                  ...
                  <img
                    src={
                      selectedHostel.image_path.startsWith('/uploads/')
                        ? `${api.defaults.baseURL}${selectedHostel.image_path}`
                        : `${api.defaults.baseURL}/uploads/${selectedHostel.image_path.replace(/^\/+/, '')}`
                    }
